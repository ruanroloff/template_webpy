...
from app.route.userstwo.forms import RegisterForm, LoginForm, EmailForm, PasswordForm
...
from app.utils.mailing import send_confirmation_email, send_password_reset_email, send_token_email

bpuser2 = Blueprint("user", __name__)
...
@bpuser2.route('/register', methods=['GET', 'POST'])
def register():
    ...
    return render_template('/zuser/register.html', form=form)


@bpuser2.route('/login', methods=['GET', 'POST'])
def login():
    ...
    return render_template('/zuser/form_login.html', form=form)


@bpuser2.route('/logout')
@login_required
def logout():
    user = current_user
...
@bpuser2.route('/reset', methods=['GET', 'POST'])
def reset():
    form = EmailForm()
    if request.method == 'POST':
        if form.validate_on_submit():
    ...
    return render_template('/zuser/form_password_reset.html', form=form)


@bpuser2.route('/reset/<token>', methods=['GET', 'POST'])
def reset_with_token(token):
    ...
    return render_template('/zuser/form_reset_password_with_token.html', form=form, token=token)


@bpuser2.route('/user_profile')
@login_required
def user_profile():
    return render_template('/zuser/user_profile.html')


@bpuser2.route('/email_change', methods=['GET', 'POST'])
@login_required
def user_email_change():
    ...
    return render_template('/zuser/form_change_email.html', form=form)


@bpuser2.route('/password_change', methods=['GET', 'POST'])
@login_required
def user_password_change():
    ...
    return render_template('/zuser/form_password_change.html', form=form)


@bpuser2.route('/admin_view_users')
@login_required
def admin_view_users():
    if current_user.role != 'admin':
        ...
        return render_template('/zuser/view_users.html', users=users)
